...
# ---------------------------
# 13. פונקצית בדיקה שלי לטובת בדיקת הפרמטרים 
# ---------------------------

def inspect_employee(df, employee_id, export_excel=True):
    """
    מציג ו(אופציונלית) מייצא את כל רכיבי החישוב לעובד בודד.
    """
    rec = df[df["employee_id"] == employee_id]
    if rec.empty:
        print(f"⚠️  עובד {employee_id} לא נמצא.")
        return

    r = rec.iloc[0]

    # חישובי צד נכסים שלא חישבנו בקוד המקורי
    expected_return = r["Assets_open"] * r["DiscRate"]
    # נניח שהפקדות לנכסי התוכנית ו-BenefitsPaid הם אפס אם אין נתון
    contributions = r.get("ContribAssets", 0)
    benefits_from_assets = r.get("BenefitsAssets", 0)
    asset_gain_loss = (
        r["Assets_open"] + expected_return + contributions
        - benefits_from_assets - r["PV_close"]
    )

    # הדפסה למסך
    print("\n" + "="*60)
    print(f"נתוני IAS 19 לעובד {employee_id}")
    print("="*60)
    print(f"פקטור אקטוארי = PV_close ÷ (שכר אחרון × ותק × (1-אחוז סעיף 14))")
    print(f"              = {r.PV_close:,.0f} ÷ "
          f"({r.LastSalary:,.0f} × {r.Seniority} × (1-{r.Section14Pct:.2%}))")
    print(f"              = {r.ActFactor:.6f}\n")

    print(f"עלות שירות שוטף (SC)..............: {r.SC:,.0f}")
    print(f"עלות היוון (IC)....................: {r.IC:,.0f} "
          f"(שיעור היוון {r.DiscRate:.2%})")
    print(f"הטבות ששולמו.......................: {r.BenefitsPaid:,.0f}")
    print(f"הפסד / רווח אקטוארי על התחייבות...: {r.LiabGainLoss:,.0f}\n")

    print("----- צד נכסים -----")
    print(f"שווי הוגן פתיחה....................: {r.Assets_open:,.0f}")
    print(f"תשואה צפויה על נכסים..............: {expected_return:,.0f}")
    print(f"הפקדות לנכסים......................: {contributions:,.0f}")
    print(f"הטבות ששולמו מנכסים...............: {benefits_from_assets:,.0f}")
    print(f"הפסד / רווח אקטוארי על נכסים.......: {asset_gain_loss:,.0f}")
    print("="*60 + "\n")
    ...
    if export_excel:
        out = pd.DataFrame({
            "מספר עובד":               [employee_id],
            "שכר אחרון":               [r.LastSalary],
            "וותק (שנים)":             [r.Seniority],
            "אחוז סעיף 14":            [r.Section14Pct],
            "פקטור אקטוארי":           [r.ActFactor],
            "SC – עלות שירות":         [r.SC],
            "IC – עלות היוון":          [r.IC],
            "BenefitsPaid – הטבות":    [r.BenefitsPaid],
            "רווח/הפסד אקטוא. התחייבות": [r.LiabGainLoss],
            "שווי נכסים פתיחה":        [r.Assets_open],
            "תשואה צפויה על נכסים":     [expected_return],
            "רווח/הפסד אקטוא. נכסים":   [asset_gain_loss],
        })

        fname = f"IAS19_employee_{employee_id}.xlsx"
        out.to_excel(fname, index=False)
        print(f"✓ קובץ הסיכום נשמר כ-{fname} "
              f"(מיקום: {os.path.abspath(fname)})")


if __name__ == "__main__":
    main()

    # טוען את התוצאה שחישבת זה עתה
    df_full = pd.read_excel("IAS19_partB_results.xlsx")

    while True:
        emp = input("הקלד מספר עובד לבדיקת פירוט (או Enter לסיום): ").strip()
        if not emp:
            break
        if not emp.isdigit():
            print("❌ נא להקליד מספר עובד תקין.")
            continue
        inspect_employee(df_full, int(emp))